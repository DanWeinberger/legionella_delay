...
for(t in 32:N_times){ #22 lag from linking epsilon to lambda, and 10 lag from linking rainfall to epsilon
 ...
 #  alpha[1] ~ dnorm(0,(1-rho2^2)*tau2.beta)
 # for(i in 2:N_times){
 #  alpha[i] ~ dnorm(rho2*alpha[i-1], tau2.beta) #AR(1) model
 # }
...
params<-c('lambda', 'beta','delay_est','epsilon', 'rho3','tau3.beta', 'int')
