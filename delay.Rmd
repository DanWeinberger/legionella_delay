...
rain_df$rain2 <- lag(rain_df$rain1,2)
rain_df$rain3 <- lag(rain_df$rain1,3)
rain_df$rain4 <- lag(rain_df$rain1,4)
rain_df$rain5 <- lag(rain_df$rain1,5)
rain_df$rain6 <- lag(rain_df$rain1,6)
rain_df$rain7 <- lag(rain_df$rain1,7)
rain_df$rain8 <- lag(rain_df$rain1,8)
rain_df$rain9 <- lag(rain_df$rain1,9)
rain_df$rain10 <- lag(rain_df$rain1,10)
...
for(t in 32:N_times){ #22 lag from linking epsilon to lambda, and 10 lag from linking rainfall to epsilon
  ...
  int ~ dnorm(0, 1e-4)
 alpha[1] ~ dnorm(0, tau2.alpha/(1-rho2^2))

 for( t in 2:N_times ){
  alpha[t] ~ dnorm(rho2*alpha[t-1],tau2.alpha)
 }
 
 beta[1] ~ dnorm(0,tau3.beta/(1-rho3^2))
 for(i in 2:10){
