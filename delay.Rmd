...
issues--
1) use dirichlet to capture uncertainty in the delay distribution
```{r}
model_string <- "
model{
for(t in 22:N_times){
...
for(d in (1:21)){ #triangle incomplete for first 21 time points
  delta[(t+d),d] <- epsilon[t] * delay_est[d] #the t+d is the key here
}
 ...
 for( t in 2:N_times ){
  ...
  phi[t]~ dnorm(0, tau1)
   ...
   delay_est ~ ddirch(delay_dist) #use values from literature as priors for the dirichlet
...
params<-c('lambda','epsilon', 'alpha', 'phi','delay_est')
...
plot(post_delay_est$mean, type='l')
points(delay_dist)
...
matplot(t(post_samples_delay[1:100,]))
