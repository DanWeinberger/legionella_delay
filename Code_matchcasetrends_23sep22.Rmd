...
rain_weights <- c(0,0, 0, 0, 0.1,0.2,0.5,0.2,0,0,0,0,0,0,0) 
rain_composite = as.matrix(w.2017[,-1]) %*% rain_weights

set.seed(123)

rain_comp_weight = 1 # this helps model fitting below. 
...
mod1 <- glm(N_exposures ~  sin1 +cos1 +rain0 + rain1 + rain2 + rain3 + rain4 + rain5 + rain6 + rain7 + rain8 + rain9 , data=df1, family='poisson')
summary(mod1)
#coefs_rain <- coef(mod1)[-c(1,2,3)]
coefs_rain <- coef(mod1)[grep('rain',names(coef(mod1)))] 

plot(coefs_rain)
points(rain_weights, col='red')
